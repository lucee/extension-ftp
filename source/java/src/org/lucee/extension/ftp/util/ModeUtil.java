...
public final class ModeUtil {
	...
	private static int _toOctalMode(String strMode) {
		...
		// owner
		if ("r".equals(strMode.substring(index++, index)))
			mode += 0400;
		if ("w".equals(strMode.substring(index++, index)))
			mode += 0200;
		if ("x".equals(strMode.substring(index++, index)))
			mode += 0100;
		// group
		if ("r".equals(strMode.substring(index++, index)))
			mode += 040;
	...
	public static String fromOctalMode(int mode) throws IOException {
		...
		if (octalString.length() <= 4 && octalString.length() > 3)
			throw new IOException(
					"Can't translate [" + mode + "] to a permissions string, [" + octalString + "] is invalid");
		String permString = "";
	...
	/**
	 * Extracts the permission bits from the mode value. Logs a message if the file
	 * type bits are removed and logging is enabled.
	 * 
	 * @param mode
	 *            The mode value that includes file type and permission bits.
	 * @param log
	 *            If true, log a message if file type bits are removed.
	...
	public static int extractPermissions(int mode, boolean log) {
		...
		if (log && (mode != permissionBits)) {
			LogUtil.log(Log.LEVEL_WARN, "File type bits removed from mode: original=" + toStringMode(mode)
					+ ", extracted=" + toStringMode(permissionBits));
		}
