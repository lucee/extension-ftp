...
import org.lucee.extension.ftp.util.SimpleDumpData;
...
public final class FTPPath implements Dumpable {
	...
	public FTPPath(AFTPClient client, String relpath) throws PageException, IOException {
		...
		Array relpathArr = CFMLEngineFactory.getInstance().getListUtil().toArrayTrim(relpath, "/");
		...
		if (client == null)
			...
			current = client.printWorkingDirectory().replace('\\', '/');
		Array parentArr = CFMLEngineFactory.getInstance().getListUtil().toArrayTrim(current, "/");
		...
		append(parentArr, relpathArr);
		init(parentArr);
	}

	private static void append(Array target, Array source) throws PageException {
	...
	private void init(Array arr) throws PageException {
		if (arr.size() > 0) {
			...
			this.path = '/' + CFMLEngineFactory.getInstance().getListUtil().toList(arr, "/") + '/';
		} else {
			this.path = "/";
			this.name = "";
		}
	...
	public String toString() {
		return path + name;// +" -
							// "+"path("+getPath()+");"+"name("+getName()+");"+"parent("+getParentPath()+");";
	}
