...
public abstract class AFTPClient {
	...
	public static AFTPClient getInstance(String secure, InetAddress host, int port, String username, String password,
			String fingerprint, boolean stopOnError) throws IOException {

		AFTPClient client;

		if (secure.equals("FTPS"))
			client = new FTPSClientImpl(); // FTPS
		else if (CFMLEngineFactory.getInstance().getCastUtil().toBooleanValue(secure, false))
			client = new SFTPClientImpl(); // SFTP
		else
			client = new FTPClientImpl(); // FTP
	...
	/***
	 * Returns the integer value of the reply code of the last FTP reply. You will
	 * usually only use this method after you connect to the FTP server to check
	 * that the connection was successful since <code> connect </code> is of type
	 * void.
	 * <p>
	 * 
	 ...
	 *            may well cause this method to fail. Also, some servers treat a
	 *            leading '-' as being an option. To avoid this interpretation, use
	 *            an absolute pathname or prefix the pathname with ./ (unix style
	 *            servers). Some servers may support "--" as meaning end of options,
	 ...
	 * <p>
	 * Note: if you have used setRestartOffset(long), the file data will start from
	 * the selected offset.
	 * 
	 * @param remote
	...
	/***
	 * A convenience method to send the FTP QUIT command to the server, receive the
	...
	public abstract void init(InetAddress host, int port, String username, String password, String fingerprint,
	 ...
	 * returning, connectAction is called to perform connection initialization
	 * actions.
	 * <p>
	 * 
	 ...
	 * @exception IOException
	 *                If the socket could not be opened. In most cases you will only
	 *                want to catch IOException since SocketException is derived
	 *                from it.
	 ...
	 * send a positive completion response on the final successful completion of a
	 * command.
	 * <p>
	 *
	...
	/**
	 * Sends a NOOP command to the server. This is useful for keep-alive and
	 * connection validation purposes.
	 * <p>
	 * 
	 ...
	 *                receiving the server reply.
	 */
	public abstract boolean sendNoOp() throws IOException;
}
