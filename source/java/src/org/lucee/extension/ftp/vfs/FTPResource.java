...
import org.lucee.extension.ftp.util.ModeUtil;
...
public final class FTPResource extends ResourceSupport {
	...
	FTPResource(FTPResourceProvider provider, FTPConnectionData data, String path) {
		super(CFMLEngineFactory.getInstance());
	...
	private Boolean hasPermission(int permission) {
		...
		try {
			...
			return engine.getCastUtil()
					.toBoolean(file.hasPermission(FTPFile.USER_ACCESS, permission)
	...
	public boolean setLastModified(long time) {
		...
		try {
			...
			Calendar c = ReflectionUtil.JREDateTimeUtil_getThreadCalendar();
			if (pc != null)
	...
	public void moveTo(Resource dest) throws IOException {
		FTPResourceClient client = null;
		engine.getResourceUtil().checkMoveToOK(this, dest);
		try {
			provider.lock(this);
			client = provider.getClient(data);
			...
			if (dest instanceof FTPResource)
				moveTo(client, (FTPResource) dest);
			else
				super.moveTo(dest);
	...
	public InputStream getInputStream() throws IOException {
		...
		try {
			return engine.getIOUtil().toBufferedInputStream(
					new FTPResourceInputStream(client, this, client.retrieveFileStream(getInnerPath())));
		} catch (IOException e) {
			provider.returnClient(client);
			provider.unlock(this);
			throw e;
	...
	public OutputStream getOutputStream(boolean append) throws IOException {
		...
		try {
			...
			if (os == null)
				throw new IOException("Can't open stream to file [" + this + "]");

			return engine.getIOUtil().toBufferedOutputStream(new FTPResourceOutputStream(client, this, os));
		} catch (IOException e) {
			provider.returnClient(client);
			provider.unlock(this);
			throw e;
	...
	boolean isRoot() {
		return engine.getStringUtil().isEmpty(name);
	}
