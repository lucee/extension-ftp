...
public final class FTPResourceClient extends FTPClient {
	...
	private final Map<String, FTPFileWrap> files = new ConcurrentHashMap<>();
	...
	public boolean changeWorkingDirectory(String pathname) throws IOException {
		if (pathname != null && pathname.endsWith("/") && pathname.length() != 1)
			pathname = pathname.substring(0, pathname.length() - 1);

		if (pathname.equals(workingDirectory))
			return true;
