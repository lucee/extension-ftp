...
public abstract class ResourceSupport implements Resource {
	...
	private static void checkMoveToOK(Resource source, Resource target) throws IOException {
		if (!source.exists()) {
			throw new IOException(
					"can't move [" + source.getPath() + "] to [" + target.getPath() + "], source file does not exist");
		}
		if (source.isDirectory() && target.isFile())
			throw new IOException(
					"can't move [" + source.getPath() + "] directory to [" + target.getPath() + "], target is a file");
		if (source.isFile() && target.isDirectory())
			throw new IOException(
					"can't move [" + source.getPath() + "] file to [" + target.getPath() + "], target is a directory");
	}
	...
	public String[] list(ResourceNameFilter filter) {
		...
		for (int i = 0; i < lst.length; i++) {
			if (filter.accept(getParentResource(), lst[i]))
				list.add(lst[i]);
		}
		if (list.size() == 0)
			return new String[0];
	...
	public Resource[] listResources(ResourceNameFilter filter) {
		...
		for (int i = 0; i < files.length; i++) {
			if (filter.accept(this, files[i]))
				list.add(getRealResource(files[i]));
		}
		return list.toArray(new Resource[list.size()]);
	}
	...
	public boolean equals(Object obj) {
		...
		if (getResourceProvider() != other.getResourceProvider())
			return false;
		...
		if (getPath().equalsIgnoreCase(other.getPath()))
			return true;
