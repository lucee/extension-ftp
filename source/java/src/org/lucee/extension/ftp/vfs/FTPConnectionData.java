...
public final class FTPConnectionData implements IFTPConnectionData {
	...
	public FTPConnectionData(String host, String username, String password, int port, boolean customHostPort,
			boolean customUserPass) {
		this.host = host;
	...
	public static DataAndPath load(IFTPConnectionData base, String path) {
		...
		return new DataAndPath(new FTPConnectionData(host, username, password, port, customHostPort, customUserPass),
	...
	public String toString() {
		return new StringBuilder().append("username:").append(username).append(";password:").append(password)
				.append(";hostname:").append(host).append(";port:").append(port).toString();
	}
	...
	public String key() {
		...
		if (!Util.isEmpty(username) && customUserPass)
	...
	public boolean hasProxyData() {
		if (data == null || Util.isEmpty(data.getServer(), true)) {
			return false;
		}
		return true;
	}
